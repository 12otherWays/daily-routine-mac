...
- **Month labels**: positioned by absolute x-offset (`columnX`) above the column where each month starts (`monthStarts`), free-floating (`fixedSize`) in a `ZStack` so a label wider than a cell overflows cleanly without breaking grid alignment. A 22pt min-gap prevents a left-edge partial month from crowding the next label.
